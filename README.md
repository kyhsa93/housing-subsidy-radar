...
`docs/analytics.js` — GA4 로더(`G-Z1LH7S1ZE5`). 블로그·다른 프로젝트 페이지와 같은 속성을 쓰기
때문에, 페이지의 `<meta name="site-group">` 값을 `content_group`으로 실어 보내야 보고서에서 갈린다.
페이지뷰는 자동 전송을 끄고 렌더가 끝난 뒤(`main()`)에 보낸다 — 렌더 전에 보내면 제목이 정해지기
전이고, 렌더가 실패해 아무도 안 부르면 로더 안 타이머가 대신 보낸다. 데이터 로드 실패는 `exception`
이벤트로 남긴다. **광고(애드센스)는 붙이지 않는다** — 크롤러가 보는 본문이 아직 얇고, 심사는 도메인
단위라 이 페이지 하나가 도메인 전체에 영향을 준다.

`scripts/prerender.mjs` — `docs/data/cheongyak.json`을 읽어 공고 목록을 `docs/index.html`의
자리표시 주석 사이에 심는다. 손대지 않으면 크롤러가 받는 HTML에는 "불러오는 중..."밖에 없다.
CI가 수집 직후·테스트 직전에 실행하고 `docs/index.html`도 함께 커밋한다.

일부러 하지 않는 두 가지가 있다. **지원금 목록은 심지 않는다** — 그 패널은 처음에 `hidden`이라
심으면 화면에 없는 걸 크롤러에만 보여주는 꼴이 된다(검색에 태우려면 별도 URL로 빼는 게 맞다).
**D-day도 넣지 않는다** — 만든 시점의 "오늘"에 좌우돼서, 데이터가 그대로여도 하루 뒤엔 결과가
달라지고 그러면 "커밋된 HTML이 데이터와 맞는가"를 검사할 수 없다.
